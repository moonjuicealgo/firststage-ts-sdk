...
async listFirstStageAssets(): Promise<{ assetId: number; info: AssetInformation }[]> {
  ...
  const assetIds: number[] = (accountInfo.assets ?? []).map((a: any) => Number(a['assetId']));

  const result: { assetId: number; info: AssetInformation }[] = [];
...
if (fromAssetInfo && toAssetInfo) {
  ...
  if (quote.usdIn) quote.usdIn /= 1 - taxRate;
}
...
if (toAssetInfo?.buy_tax && toAssetInfo.total_tax_bps && type === "fixed-input") {
  ...
  if (quote.usdOut) quote.usdOut /= 1 - buyTaxRate;
}
 ...
 */
async isFrozen(userAddress: string, assetId: number): Promise<boolean> {
  try {
...
async generateGeneralOpsGroup(
  ...
  // Grouped txns
  const freezeTxns: UserTxnWithSigner[] = [];
  const topCalls: any[] = [];
  const bottomCalls: any[] = [];
  const optIns: UserTxnWithSigner[] = [];
    ...
    for (const { txn } of userTxns) {
      if (txn.type === "axfer") {
        ...
        if (txAssetId === BigInt(assetId)) {
          if (axfer.sender?.toString() === userAddress) netChange -= amt;
          if (axfer.assetTransfer?.receiver?.toString() === userAddress) netChange += amt;
        }
      }
    }
...
optIns.push({ txn: optInTxn, signer: signerAccount.signer });
  ...
  console.log(
    `Freeze txns: ${freezeTxns.length}, top calls: ${topCalls.length}, swaps: ${userTxns.length}, bottom calls: ${bottomCalls.length}`
  );

  for (const { txn, signer } of optIns) {
  composer.addTransaction(txn, signer);
};
  for (const top of topCalls) composer.addAppCallMethodCall(top);
  for (const { txn, signer } of userTxns) composer.addTransaction(txn, signer ?? signerAccount.signer);
  for (const bottom of bottomCalls) composer.addAppCallMethodCall(bottom);

  return composer;
}

async wrapDeflexSwapGroup(
  apiResponse: FirstStageSwapTxnsResponse,
  userAddress: string,
  referralAddress: string,
  fromAssetId: number,
  toAssetId: number,
  ...
  const deflexTxns = apiResponse.txns.map((t) =>
    FirstStageDeflexTransaction.fromApiResponse(t),
  );
  ...
  async function reSignTransaction(
    ...
    signature: FirstStageDeflexTransactionSignature,
  ): Promise<Uint8Array> {
    try {
  ...
  function makeDeflexTransactionSigner(
    deflexSignature: FirstStageDeflexTransactionSignature,
  ...
  let composer: TransactionComposer;

  if (assetIds.length > 0) {
    composer = await this.generateGeneralOpsGroup(
      userAddress,
      assetIds,
      userTxns,
      referralAddress,
  ...
  async generateDeflexSwapGroup(
    quotePayload: Record<string, string> | null,
    userAddress: string,
    referralAddress: string,
    fromAssetId: number,
    toAssetId: number,
...
async generatePeraSwapGroup(
  ...
  referralAddress: string,
  slippage = 0.005,
  providers: ("tinyman" | "vestige-v4" | "tinyman-swap-router")[] = ["tinyman", "vestige-v4"],
  defaultSigner?: algosdk.TransactionSigner,
  quoteIdStr?: string
): Promise<TransactionComposer> {
  const peraSwap = new PeraSwap(this.network);
