...
var PeraSwap = class {
  ...
  async createQuote(body, signal) {
    return makeRequest(this.baseURL, "/v1/dex-swap/quotes/", {
      method: "POST",
   ...
   */
  async updateQuote(quoteId, exceptionText) {
    ...
    switch (event.data.type) {
      case "TXN_SIGN_REQUEST": {
        ...
        const decodedTxGroups = txGroups.map(
          (group) => group.map((bytes) => algosdk.decodeUnsignedTransaction(bytes))
        );
...
export { PeraSwap, SwapWidgetSearchParamKey, WidgetController };
